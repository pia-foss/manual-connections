...
set_ikev2_creds_from_api () {
    ...
    IKEV2_USERNAME=$(echo "$result" | jq -r .vpn_secret1)
    IKEV2_PASSWORD=$(echo "$result" | jq -r .vpn_secret2)
}
...
ip_passthrough_conf=""
passthrough_dest_ips_comma_separated=$(IFS=, ; echo "${PIA_PASSTHROUGH_DEST_IP_RANGES[*]}")
if [[ -n $passthrough_dest_ips_comma_separated ]]; then
    ip_passthrough_conf="
  passthrough-ips {
    remote_addrs = 127.0.0.1
    children {
      default {
        remote_ts = $passthrough_dest_ips_comma_separated
        mode = pass
        start_action = trap
      }
    }
...
extra_swan_conf=""
if declare -f PIA_EXTRA_SWAN_CONF_FUNC > /dev/null; then
    extra_swan_conf=$(PIA_EXTRA_SWAN_CONF_FUNC vpn_token_$IKEV2_USERNAME)
fi
...
# Intermediate cert doesn't seem to be deployed on PIA servers, it'll expire and fail
# For now deploy all known intermediate certs locally as well as the root ca.
# For some servers, strongswan needs to recompiled to ignore certificate.
...
max_wait_attempt=30
current_attempt=0
while ! swanctl --list-sas --ike pia-default | grep -q "pia-default:.*ESTABLISHED"; do
    if [[ $current_attempt -gt $max_wait_attempt ]]; then
        log "Main SA didn't come up"
        exit 1
    fi
...
if [[ $PIA_PF != "true" ]]; then
    log "No port forwarding. End of the script, blocking until interrupt"
    cat
else
